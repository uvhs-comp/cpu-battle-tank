...
var BattleTank = function() {
	...
	var Tank = function(tankId, x, y, color, grid) {
		...
		this.fire = function() {
			if(this.ammo > 0) {
				if (!bt.settings.unlimitedMode) this.ammo--;
			} else {
				if(!bt.settings.unlimitedMode) throw this.ERR_OUT_OF_AMMO;
			}
		...
		this.moveForwards = function() {
			...
			if(this.fuel < 1 && !bt.settings.unlimitedMode)
			...
			if (!bt.settings.unlimitedMode) this.fuel--;
		}
	...
	var Grid = function(size){
		...
		// setup game
		this.init = function() {
			this.updateStatus("Welcome to Battle Turtles");
			this.drawGrid();

			// setup tank movement callback timer
			setInterval(this.moveTanks, 1000);
			html = '<div id="welcome"><h3>Welcome to CPU Battle Turtles</h3><p>This puzzle is designed as a teaching aid to help you understand how the Fetch / Execute cycle works inside a Central Processing Unit. </p><p>In the practice level the complete code is written for you to move the tank forward 2 squares.</p><p>Click on the tank to get started.</p></div>';
			$('#dlg').html(html);
	...
	this.addTank = function(id, x,y, color, code) {
		var html = '<div id="tank_' + id + '" class="tank">\
        	<div class="inner_tank">\
                <img class="tank_base" src="media/turtle-base.png" />\
                <div class="tank_top" >\
	...
	this.settings = {
		...
		tankMode:true,
		showGrid:true,
		controller:this.CONTROLLER_LITTLE_MAN_COMPUTER,
		runSpeed:10
	}
	...
	this.onSettingChange = function(e) {
		...
		$('.tank_unlimited').css({display:(bt.settings.unlimitedMode?'block':'none')});
		$('.tank_mode').css({display:(bt.settings.tankMode?'block':'none')});
